#!/usr/bin/env python
# train_3DL_multi_wells.py  ——  多井位深度学习同时建模预测
import argparse, os, random, logging, warnings, sys
...
# 1. 单井位深度学习建模函数
# ----------------------------------------------------------------------
def fit_single_well_dl(well_data, well_name, cfg, well_index, model_type, device):
    """
    单井位深度学习建模函数，用于并行处理
    ...
    try:
        ...
        if np.isnan(ser).all() or len(ser[~np.isnan(ser)]) < cfg.window_size + cfg.pred_len + 20:
        ...
        # 创建模型
        model = get_model(model_type, cfg.window_size, cfg.pred_len, device)
        ...
        for ep in range(1, cfg.epochs + 1):
            ...
            for xb, yb in train_loader:
                ...
                out = model(xb)
                loss = loss_with_derivative(out, yb, alpha=cfg.alpha)
                optim.zero_grad()
                loss.backward()
            ...
            # 反归一化
            preds_inv = scaler.inverse_transform(preds.reshape(-1, 1)).reshape(-1, cfg.pred_len)
            trues_inv = scaler.inverse_transform(trues.reshape(-1, 1)).reshape(-1, cfg.pred_len)
        ...
        if original_index is not None:
            ...
            train_index = np.arange(cfg.window_size, cfg.window_size + len(pred_tr))
            test_index = np.arange(cfg.window_size + len(pred_tr), 
...
def plot_individual_well_results(results, output_dir, show_wells=None, log=None):
    ...
    for well_name, model_type, result in results_to_plot:
        ...
        ax.plot(pred_data['train_loss_hist'], label='Train Loss', color='blue', alpha=0.7)
        ax.plot(pred_data['test_loss_hist'], label='Test Loss', color='red', alpha=0.7)
        ax.set_xlabel('Epoch')
        ax.set_ylabel('MSE Loss')
        ax.set_title(f'{well_name} - {model_type} 训练损失曲线')
        ax.legend()
        ax.grid(True, alpha=0.3)
        
        loss_path = f"{output_dir}/loss_curve_{model_type}_{well_name}.png"
        fig.tight_layout()
        fig.savefig(loss_path, dpi=300, bbox_inches='tight')
        plt.close()
        
        if log:
            log.info(f"  ✅ {well_name} - {model_type} 图表已保存: {pred_path}, {loss_path}")
    
    if log:
        log.info(f"单井位预测图表生成完成，共 {len(results_to_plot)} 个结果")
...
def main(cfg):
    ...
    for idx, col in enumerate(well_columns):
        ...
        for model_type in selected_models:
            tasks.append((df[col], col, cfg, well_index, model_type, device))
    ...
    if cfg.parallel:
        ...
        with ProcessPoolExecutor(max_workers=cfg.n_jobs) as executor:
            # 提交任务
            future_to_task = {
                executor.submit(fit_single_well_dl, *task): task 
                for task in tasks
            }
        ...
        for task in tasks:
            well_name, model_type, result = fit_single_well_dl(*task)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="多井位深度学习时序预测建模")
    ...
    parser.add_argument("--models", type=str, default="transformer",
                        help="指定要使用的模型，用逗号分隔，如 'gru,lstm' 或 'transformer'；'all'表示使用所有模型")
